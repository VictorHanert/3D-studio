...
import type { CollisionMode } from '@/system/utilities/CollisionDetection';
...
const selectedCollisionMode = ref<CollisionMode>('aabb');
...
onMounted(async () => {
    ...
    try {
        ...
        selectedCollisionMode.value = planner.state.collisionMode;
...
const handleCollisionModeChange = (mode: CollisionMode): void => {
    selectedCollisionMode.value = mode;
    planner.setCollisionMode(mode);
};

const resetCollisionMetrics = (): void => {
    planner.resetCollisionMetrics();
    planner.resetFrameMetrics();
};

const logCollisionMetrics = (): void => {
    const report = planner.getBenchmarkReport();
    const mode = report.collisionMode;
    const spawn = report.collision.spawn?.[mode];
    const interaction = report.collision.interaction?.[mode];

    console.group(`Benchmark Report (${mode.toUpperCase()})`);
    ...
    console.table([
        {
            domain: 'spawn',
            mode,
            samples: spawn?.samples ?? 0,
            avgMs: Number((spawn?.averageMs ?? 0).toFixed(4)),
            p50Ms: Number((spawn?.p50Ms ?? 0).toFixed(4)),
            p95Ms: Number((spawn?.p95Ms ?? 0).toFixed(4)),
            minMs: Number((spawn?.minMs ?? 0).toFixed(4)),
            maxMs: Number((spawn?.maxMs ?? 0).toFixed(4)),
            stdDevMs: Number((spawn?.stdDevMs ?? 0).toFixed(4)),
            lastMs: Number((spawn?.lastMs ?? 0).toFixed(4)),
        },
        {
            domain: 'interaction',
            mode,
            samples: interaction?.samples ?? 0,
            avgMs: Number((interaction?.averageMs ?? 0).toFixed(4)),
            p50Ms: Number((interaction?.p50Ms ?? 0).toFixed(4)),
            p95Ms: Number((interaction?.p95Ms ?? 0).toFixed(4)),
            minMs: Number((interaction?.minMs ?? 0).toFixed(4)),
            maxMs: Number((interaction?.maxMs ?? 0).toFixed(4)),
            stdDevMs: Number((interaction?.stdDevMs ?? 0).toFixed(4)),
            lastMs: Number((interaction?.lastMs ?? 0).toFixed(4)),
        },
    ]);
    ...
    console.table([
        {
            domain: 'frame',
            samples: report.frame.samples,
            avgFrameMs: Number(report.frame.averageFrameMs.toFixed(4)),
            p50FrameMs: Number(report.frame.p50FrameMs.toFixed(4)),
            p95FrameMs: Number(report.frame.p95FrameMs.toFixed(4)),
            minFrameMs: Number(report.frame.minFrameMs.toFixed(4)),
            maxFrameMs: Number(report.frame.maxFrameMs.toFixed(4)),
            avgFps: Number(report.frame.averageFps.toFixed(2)),
            p50Fps: Number(report.frame.p50Fps.toFixed(2)),
            p95Fps: Number(report.frame.p95Fps.toFixed(2)),
        },
    ]);

    if (report.warnings.length > 0) {
        console.warn('Benchmark warnings:', report.warnings);
    }

    console.log('Raw report JSON:', JSON.stringify(report, null, 2));
    console.groupEnd();
};
    ...
    <div :class="[
        ...
        <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex-shrink-0 flex items-center justify-between">
            ...
            <button @click="togglePanel"
                ...
                <Lineicons :icon="isCollapsed ? AngleDoubleRightOutlined : AngleDoubleLeftOutlined" :size="24"
                    :stroke-width="2" />
            </button>
        </div>
        ...
        <div v-if="!isCollapsed" class="flex-1 flex flex-col overflow-hidden">
            ...
            <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
                ...
                <select v-model="selectedGroundTexture" @change="handleGroundTextureChange(selectedGroundTexture)"
            ...
            <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
                <label class="text-xs font-medium text-gray-700 dark:text-gray-300 block mb-2">Collision Mode</label>
                <select v-model="selectedCollisionMode" @change="handleCollisionModeChange(selectedCollisionMode)"
                    class="w-full px-3 py-2 text-sm bg-white dark:bg-gray-700 cursor-pointer border border-gray-300 dark:border-gray-600 rounded text-gray-900 dark:text-white hover:border-blue-400 focus:border-blue-500 focus:outline-none transition-colors">
                    <option value="aabb">AABB (faster, axis-aligned)</option>
                    <option value="obb">OBB (more precise, oriented)</option>
                </select>
                <div class="mt-2 flex gap-2">
                    <button @click="resetCollisionMetrics"
                        class="flex-1 px-2 py-1 text-xs rounded border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors cursor-pointer">
                        Reset All Metrics
                    </button>
                    <button @click="logCollisionMetrics"
                        class="flex-1 px-2 py-1 text-xs rounded border border-blue-300 dark:border-blue-600 text-blue-700 dark:text-blue-300 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors cursor-pointer">
                        Log Benchmark
                    </button>
                </div>
            </div>
            ...
            <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
                ...
                <div class="flex gap-3 items-center">
                    ...
                    <!-- Material Preview -->
                    <div
                        class="w-10 h-10 flex-shrink-0 bg-gray-100 dark:bg-gray-700 rounded border border-gray-300 dark:border-gray-600 flex items-center justify-center overflow-hidden">
                        <img v-if="materialPreview" :src="materialPreview" :alt="selectedMaterial"
                            class="w-full h-full object-cover" />
                        <svg v-else class="h-5 w-5 text-gray-400 dark:text-gray-500 animate-pulse" fill="none"
            ...
            <div class="flex-1 overflow-y-auto">
                <div class="p-4 space-y-3">
                    <div v-if="loadingModules" class="text-center py-8 text-gray-500">
                        <svg class="h-6 w-6 text-gray-400 animate-spin mx-auto mb-2" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <button v-for="module in modules" v-else :key="module.folder" @click="handleModuleClick(module)"
                        ...
                        <!-- Module Thumbnail -->
                        <div
                            class="aspect-video bg-gray-100 dark:bg-gray-700 rounded-md mb-2 flex items-center justify-center overflow-hidden">
                            <img v-if="module.thumbnail" :src="module.thumbnail" :alt="module.name"
                                class="w-full h-full object-cover" />
                            <svg v-else class="h-8 w-8 text-gray-400 dark:text-gray-500 animate-pulse" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
